...
def compute_prog(input, p=None, start=0):
    ...
    while True:
        try:
            A[tuple(X)] # Initialize for the printer
            print_block(A, X, dir)
...
def print_block(A, X, dir):
    ...
    S = ""
    for y in reversed(range(min([i[1] for i in A]), max([i[1] for i in A]) + 1)):
        for x in range(min([i[0] for i in A]), max([i[0] for i in A]) + 1):
            entry = A[x, y]
            if (x, y) == tuple(X):
                if entry in [0, 1]:
...
def main():
    ...
    if sys.argv[1:] == ['0']:
            print("Test input")
            A = compute_prog(input, p=test_prog())
            print(np.sum(abs(np.array(list(A.values())))))
    elif sys.argv[1:] == ['1']:
        A = compute_prog(input)
        print(np.sum(abs(np.array(list(A.values())))))
    elif sys.argv[1:] == ['2']:
        A = compute_prog(input, start=-1)
        # print(np.sum(abs(np.array(list(A.values())))))
    else:
        sys.exit("Error: must run 'python day11.py 1' or 'python day 11.py 2'.")
