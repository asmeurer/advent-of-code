...
_BAGS = r"(?:\w+ \w+) bags?"
BAGS = r"(\w+ \w+) bags?"
NBAGS = re.compile(rf"(\d+) {BAGS}")
EMPTY_RE = re.compile(rf"{BAGS} contain no other bags\.")
CONTAINS_RE = re.compile(rf"{BAGS} contain ((?:\d+ {_BAGS}(?:, )?)+)\.")

my_bag = "shiny gold"

def get_bag_tree(text):
    tree = {}
    for line in text.strip().splitlines():
        if m := EMPTY_RE.match(line):
            tree[m.group(1)] = []
        elif m := CONTAINS_RE.match(line):
            tree[m.group(1)] = NBAGS.findall(m.group(2))
        else:
            raise ValueError(f"Didn't match {line}")

    return tree

def traverse(tree, bag):
    for n, inner in tree[bag]:
        yield (n, inner)
        yield from traverse(tree, inner)

def count_outermost_bag(tree):
    N = 0
    for outermost in tree:
        for n, bag in traverse(tree, outermost):
            if bag == my_bag:
                N += 1
                break
    return N
