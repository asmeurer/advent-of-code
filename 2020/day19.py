...
def create_grammar(grammar):
    ...
    while i >= 0:
        if str(i) in grammar:
            grammar = grammar.replace(str(i), name(i))
        i -= 1
    return lark.Lark(grammar, start=name(0))

def check_inputs(lark_grammar, inputs):
    matching = []
    for m in inputs:
        try:
            lark_grammar.parse(m)
            matching.append(m)
...
print("Part 1")
print("Test input")
test_grammar, test_inputs = parse_input(test_input)
